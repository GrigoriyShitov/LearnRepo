...
@command(
    "list product_categories", "список всех категорий", CATEGORY_PRODUCT_CATEGORIES
)
...
def show_product_category(_id: str) -> None:
    ...
    with conn.cursor(row_factory=class_row(ProductCategory)) as cur:
        cur.execute(
            "SELECT id, name FROM catalog.product_categories WHERE id = %s", (_id,)
        )
...
def add_product_category() -> None:
    ...
    try:
        ...
        if "unique" in str(e).lower():
            render_error(f"Категория с названием «{name}» уже существует")
            return
        raise
...
@command(
    "edit product_category", "редактировать категорию", CATEGORY_PRODUCT_CATEGORIES
)
...
@command("delete product_category", "удалить категорию", CATEGORY_PRODUCT_CATEGORIES)
